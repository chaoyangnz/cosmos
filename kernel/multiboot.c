...
multiboot__setup() {
    ...
    for(int i = 0; i < sizeof(multiboot_info_t); ++i) {
        *multiboot_info_ptr++ = *multiboot_info_ptr_origin++;
    }
    // copy and fix address
    multiboot_info_ptr->mmap_addr = multiboot_info_ptr_origin->mmap_addr;
    multiboot_info_ptr->framebuffer_addr = multiboot_info_ptr_origin->framebuffer_addr;
}
